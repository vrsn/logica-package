...
from .common import color, concertina_lib, sqlite3_logica
from .compiler import rule_translate, universe
...
def RunSQL(sql, engine, connection=None, is_final=False):
    if engine == "bigquery":
        ...
        raise Exception(
            "Logica only supports BigQuery, PostgreSQL and SQLite " "for now."
        )
...
def PostgresJumpStart():
    ...
    if result != 0:
        print(
            """Installation failed. Please try the following manually:
# Install Logica.
!pip install logica
...
colab_logica.SetDbConnection(connection)"""
    ...
    engine = create_engine(
        "postgresql+psycopg2://logica:logica@127.0.0.1", pool_recycle=3600
    )
    connection = engine.connect()
    print("Connected.")
