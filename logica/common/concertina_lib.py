...
class Concertina(object):
    ...
    def AsGraphViz(self):
        ...
        for a in self.all_actions:
            ...
            g.node(
                a, shape=shape, fillcolor=color, style="filled,rounded", color="gray34"
            )
...
def RenamePredicate(
    table_to_export_map, dependency_edges, data_dependency_edges, from_name, to_name
):
...
def ExecuteLogicaProgram(logica_executions, sql_runner, sql_engine):
    def ConcertinaConfig(
        table_to_export_map, dependency_edges, data_dependency_edges, final_predicates
    ):
        depends_on = {}
