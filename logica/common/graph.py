...
visjs = urllib.request.urlopen(
    "https://raw.githubusercontent.com/EvgSkv/vis/master/dist/vis-network.min.js"
).read()
css = urllib.request.urlopen(
    "https://raw.githubusercontent.com/EvgSkv/vis/master/dist/vis-network.min.css"
).read()
...
def DirectedGraphFromList(edges):
    nodes = list({n for e in edges for n in e})
    nodes_json = [{"id": n, "label": str(n)} for n in nodes]
    edges_json = [{"from": e[0], "to": e[1], "arrows": "to"} for e in edges]
    DisplayGraph(nodes_json, edges_json)
