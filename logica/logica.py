...
if __name__ == "__main__" and not __package__:
    ...
    from common import color, sqlite3_logica
else:
    from .common import color, sqlite3_logica
    from .compiler import functors, rule_translate, universe
    from .parser_py import parse
...
def main(argv):
    if len(argv) <= 1 or argv[1] == "help":
        ...
        print("")
        print("Example:")
        print("  python3 logica.py - run GoodIdea <<<' " 'GoodIdea(snack: "carrots")\'')
        return 1
    ...
    if len(argv) == 3 and argv[2] == "parse":
        ...
        if len(argv) < 4:
            print("Not enough arguments. Run 'logica help' for help.", file=sys.stderr)
            return 1
    ...
    try:
        parsed_rules = parse.ParseFile(program_text, import_root=GetImportRoot())[
            "rule"
        ]
    ...
    for predicate in predicates_list:
        ...
        if command == "run" or command == "run_to_csv":
            ...
            if engine == "bigquery":
                ...
                o = sqlite3_logica.RunSqlScript(statements_to_execute, format).encode()
            elif engine == "psql":
                p = subprocess.Popen(
                    ["psql", "--quiet"]
