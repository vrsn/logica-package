...
class HeritageAwareString(str):
    ...
    def __getitem__(self, slice_or_index) -> "HeritageAwareString":
        if isinstance(slice_or_index, int):
            ...
            stop = slice_or_index.stop if slice_or_index.stop is not None else len(self)
...
class ParsingException(Exception):
    ...
    def __init__(self, message, location):
        ...
        message = message.replace(">>", color.Color("warning")).replace(
            "<<", color.Color("end")
        )
    ...
    def ShowMessage(self, stream=sys.stderr):
        ...
        print(
            color.Format(
                ...
                dict(before=before, error_text=error, after=after),
            ),
            file=stream,
        )
...
def SplitRaw(s, separator):
    ...
    for idx, state, status in traverse:
        ...
        if (
            not state
            and s[idx : (idx + length)] == separator
            and (len(s) == idx + length or s[idx + length] != "|")
...
def SplitInOneOrTwo(s, separator):
    ...
    if len(parts) == 1:
        ...
        raise ParsingException(
            "String should have been split by >>%s<< in 1 or 2 pieces." % (separator), s
        )


def SplitMany(
    ss: List[HeritageAwareString], separator: str
) -> List[HeritageAwareString]:
...
def ParseRecordInternals(s, is_record_literal=False):
    ...
    if IsWhole(s):
        ...
        for idx, field_value in enumerate(field_values):
            if had_restof:
                raise ParsingException(
                    "Field >>..<rest_of><< must go last.", field_value
                )
            if field_value.startswith(".."):
                if is_record_literal:
                    raise ParsingException(
                        "Field >>..<rest_of> in record literals<< "
                        "is not currently suppported .",
                        field_value,
                    )
                item = {
                    "field": "*",
            ...
            if colon_split:
                ...
                if not value:
                    ...
                    if field and field[0] in string.ascii_uppercase:
                        raise ParsingException(
                            "Record fields may not start with capital "
                    ...
                    if field and field[0] == "`":
                        raise ParsingException(
                            "Backticks in variable names are "
                ...
                result.append(
                    {"field": field, "value": {"expression": ParseExpression(value)}}
                )
            else:
                ...
                if question_split:
                    ...
                    if not field:
                        raise ParsingException(
                            "Aggregated fields have to be named.", field_value
                        )
                    ...
                    if positional_ok:
                        result.append(
                            {
                                "field": idx,
                                "value": {"expression": ParseExpression(field_value)},
                            }
                        )
...
def ParseVariable(s: HeritageAwareString):
    if (
        s
        and s[0] in set(string.ascii_lowercase) | set("_")
        and set(s) <= VARIABLE_CHARS_SET
    ):
        return {"var_name": s}
...
def ParseString(s):
    ...
    if len(s) >= 6 and s[:3] == '"""' and s[-3:] == '"""' and '"""' not in s[3:-3]:
        return {"the_string": s[3:-3].replace('\\"', '"')}
...
def ParsePredicateLiteral(s):
    if (
        ...
        or s == "nil"
        or s
        and set(s) <= set(string.ascii_letters) | set(string.digits) | set(["_"])
...
def ParseInfix(s, operators=None):
    ...
    for op in operators:
        ...
        if len(parts) > 1:
            ...
            # a / b / c -> (a / b) / c
            left, right = (
                s[: parts[-2].stop - s.start],
                s[parts[-1].start - s.start :],
            )
            left = Strip(left)
            right = Strip(right)
            if op in unary_operators and not left:
                return {"predicate_name": op, "record": ParseRecordInternals(right)}
...
def BuildTreeForCombine(parsed_expression, operator, parsed_body, full_text):
    ...
    aggregated_field_value = {
        "field": "logica_value",
        "value": {"aggregation": {"operator": operator, "argument": parsed_expression}},
    }
    result = {
        "head": {
...
def ParseConciseCombine(s: HeritageAwareString):
    ...
    if len(parts) == 2:
        ...
        if len(left_parts) > 1:
            ...
            parsed_expression = ParseExpression(expression)
            parsed_body = ParseConjunction(body, allow_singleton=True) if body else None
            right_expr = BuildTreeForCombine(
                parsed_expression, operator, parsed_body, s
            )
            return {
                "left_hand_side": left_expr,
                "right_hand_side": {"combine": right_expr},
            }
...
def ParseInclusion(s):
    ...
    if len(element_list_str) == 2:
        return {
            "list": ParseExpression(element_list_str[1]),
    ...
    if s.startswith("->"):
        ...
        for (idx, state, status) in Traverse(s):
            ...
            if state == "(":
                ...
                if (
                    (idx > 0 and set(s[:idx]) <= good_chars)
                    or s[:idx] == "!"
                    or s[:idx] == "++?"
                    or idx >= 2
                    and s[0] == "`"
            ...
            return None
    if s[idx] == "(" and s[-1] == ")" and IsWhole(s[idx + 1 : -1]):
        return {
...
def ParseUnification(s):
    ...
    if len(parts) == 2:
        ...
        left_expr = ParseExpression(left)
        right_expr = ParseExpression(right)
        return {"left_hand_side": left_expr, "right_hand_side": right_expr}
...
def ParseProposition(s):
    ...
    if c:
        raise ParsingException(
            "If-then-else clause is only supported as an "
...
def ParseSubscript(s: HeritageAwareString):
    ...
    if len(str_path) >= 2:
        ...
        if not set(str_path[-1]) <= (
            set(string.ascii_lowercase) | set("_") | set(string.digits)
        ):
...
def ParseHeadCall(s):
    ...
    if len(operator_expression) == 1:
        if operator_expression[0]:
            raise ParsingException(
                "Unexpected text in the head of a rule.", operator_expression[0]
            )
        return (call, False)
    # We have a value!
    ...
    operator_str, expression_str = (operator_expression[0], operator_expression[1])
...
def ParseFunctorRule(s):
    ...
    if (
        "literal" not in new_predicate
        or "the_predicate" not in new_predicate["literal"]
    ):
...
def ParseFunctionRule(s: HeritageAwareString) -> Optional[List[Dict]]:
    ...
    annotation = ParseRule(
        HeritageAwareString("@CompileAsUdf(%s)" % this_predicate_call["predicate_name"])
    )
...
def ParseRule(s: HeritageAwareString) -> Dict:
    ...
    if len(parts) > 2:
        raise ParsingException(
            "Too many :- in a rule. " "Did you forget >>semicolon<<?", s
        )
    head = parts[0]
    ...
    if len(head_distinct) == 1:
        ...
        if not parsed_head_call:
            raise ParsingException("Could not parse head of a rule.", head)
        ...
        if not (len(head_distinct) == 2 and not head_distinct[1]):
            raise ParsingException(
                "Can not parse rule head. Something is wrong with "
    ...
    if len(parts) == 2:
        body = parts[1]
        result["body"] = {"conjunction": ParseConjunction(body, allow_singleton=True)}
...
def SplitImport(import_str):
    ...
    if len(import_path_synonym) > 2:
        raise ParsingException(
            'Too many "as":' % import_str, HeritageAwareString(import_str)
        )
    ...
    assert import_parts[-1][0].upper() == import_parts[-1][0], (
        "One import per predicate please. Violator: %s",
        import_str,
    )
...
def ParseImport(file_import_str, parsed_imports, import_chain, import_root):
    ...
    if file_import_str in parsed_imports:
        if parsed_imports[file_import_str] is None:
            raise ParsingException(
                "Circular imports are not allowed: %s."
                % "->".join(import_chain + [file_import_str]),
    ...
    if isinstance(import_root, str):
        ...
        if not os.path.exists(file_path):
            raise ParsingException(
                ...
                HeritageAwareString("import " + file_import_str + ".<PREDICATE>")[
                    7:-11
                ],
            )
    else:
    ...
    with open(file_path) as f:
        file_content = f.read()
    parsed_file = ParseFile(
        file_content, file_import_str, parsed_imports, import_chain, import_root
    )
...
def MadePredicatesRules(rules):
    ...
    for r in rules:
        if "@Make" == r["head"]["predicate_name"]:
            name = r["head"]["record"]["field_value"][0]["value"]["expression"][
                "literal"
            ]["the_predicate"]["predicate_name"]
            result[name] = r
    return result
...
class MultiBodyAggregation(object):
    ...
    def Rewrite(cls, rules):
        ...
        multi_body_aggregating_predicates = [
            n
            for n, rs in defined_predicates_rules.items()
                ...
                if name in aggregation_field_values_per_predicate:
                    ...
                    if expected_aggregation != aggregation:
                        raise ParsingException(
                            ...
                            "Signatures observed: >>%s<<"
                            % (name, str((expected_aggregation, aggregation))),
        ...
        for name in multi_body_aggregating_predicates:
            pass_field_values = [
                {
                    "field": f_v["field"],
                    "value": {"expression": {"variable": {"var_name": f_v["field"]}}},
                }
                        ...
                        "conjunct": [
                            {
                                "predicate": {
                                    ...
                                    "record": {"field_value": pass_field_values},
                                }
                            }
                        ]
                    }
    ...
    def SplitAggregation(cls, rule):
        ...
        if "distinct_denoted" not in rule:
            raise ParsingException(
                "Inconsistency in >>distinct<< denoting for "
                "predicate >>%s<<." % rule["head"]["predicate_name"],
                rule["full_text"],
            )
        del rule["distinct_denoted"]
        rule["head"]["predicate_name"] = rule["head"]["predicate_name"] + cls.SUFFIX
                ...
                aggregation_field_value = {
                    ...
                    "value": {
                        "aggregation": {
                            ...
                            "argument": {
                                "variable": {"var_name": field_value["field"]}
                            },
                        }
                    },
                }
...
class AggergationsAsExpressions(object):
    ...
    def RewriteInternal(cls, s):
        ...
        for k in member_index:
            if isinstance(s[k], dict) and "aggregation" in s[k]:
                a = s[k]["aggregation"]
...
def ParseFile(
    s, this_file_name=None, parsed_imports=None, import_chain=None, import_root=None
):
    """Parsing logica.Logica."""
    ...
    for str_statement in str_statements:
        ...
        if str_statement.startswith("import "):
            ...
            ParseImport(file_import_str, parsed_imports, import_chain, import_root)
            imported_predicates.append(
                {
                    "file": file_import_str,
                    "predicate_name": import_predicate,
                    "synonym": synonym,
                }
            )
            if file_import_str not in predicates_created_by_import:
                predicates_created_by_import[file_import_str] = DefinedPredicates(
                    parsed_imports[file_import_str]["rule"]
    ...
    if this_file_name == "main":
        ...
        for some_parsed_import in parsed_imports.values():
            if some_parsed_import:
                assert some_parsed_import["predicates_prefix"] not in existing_prefixes
    ...
    for s in imported_predicates:
        ...
        assert import_prefix, "Empty import prefix: %s -> %s" % (
        ...
        rename_count = RenamePredicate(
            rules, predicate_imported_as, import_prefix + imported_predicate_name
        )
        if (
            import_prefix + imported_predicate_name
        ) not in predicates_created_by_import[imported_predicate_file]:
            raise ParsingException(
                "Predicate %s from file %s is imported by %s, but is not defined."
                % (imported_predicate_name, imported_predicate_file, this_file_name),
                HeritageAwareString(
        ...
        for i in parsed_imports.values():
            ...
            if any(p[0] != "@" for p in defined_predicates & new_predicates):
                raise ParsingException(
                    ...
                    HeritageAwareString(str(defined_predicates & new_predicates)),
                )
