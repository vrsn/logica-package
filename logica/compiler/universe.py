...
if "." not in __package__:
    ...
    from modules.logica.compiler import (
        dialects,
        expr_translate,
        functors,
        rule_translate,
    )
    from modules.logica.parser_py import parse
else:
    from ..common import color
    from ..compiler import dialects, expr_translate, functors, rule_translate
    from ..parser_py import parse

PredicateInfo = collections.namedtuple("PredicateInfo", ["embeddable"])
Ground = collections.namedtuple("Ground", ["table_name", "overwrite"])
...
def Indent2(s):
    return "\n".join("  " + split_value for split_value in s.split("\n"))


def AnnotationError(message, annotation_value):
    raise rule_translate.RuleCompileException(message, annotation_value["__rule_text"])
...
class Annotations(object):
    ...
    def OkInjection(self, predicate_name):
        ...
        if (
            self.OrderBy(predicate_name)
            or self.LimitOf(predicate_name)
            or self.Ground(predicate_name)
            or self.NoInject(predicate_name)
            or self.ForceWith(predicate_name)
        ):
            return False
        return True
    ...
    def AttachedDatabases(self):
        ...
        for k, v in self.annotations["@AttachDatabase"].items():
            if "1" not in v:
                print(">>", v)
                AnnotationError("@AttachDatabase must have a single argument.", v)
            result[k] = v["1"]
        if (
            self.Engine() == "sqlite"
            and "logica_test" not in result
            and "@Ground" in self.annotations
            and self.annotations["@Ground"]
        ):
    ...
    def TvfSignature(self, predicate_name):
        ...
        return "CREATE TEMP TABLE FUNCTION %s(%s) AS " % (predicate_name, signature)
    ...
    def ExtractSingleton(self, annotation_name, default_value):
        ...
        if len(results) > 1:
            raise rule_translate.RuleCompileException(
                "Single %s must be provided. Provided: %s" % (annotation_name, results),
    ...
    def CheckAnnotatedObjects(self, rules):
        ...
        all_predicates = (
            {rule["head"]["predicate_name"] for rule in rules}
            | set(self.annotations["@Ground"])
            | set(self.annotations["@Make"])
        )
            ...
            if annotation_name in {
                ...
                for annotated_predicate in self.annotations[annotation_name]:
                    if annotated_predicate not in all_predicates:
                        rule_text = self.annotations[annotation_name][
                            annotated_predicate
                        ]["__rule_text"]
                        RaiseCompilerError(
                            ...
                            % (
                                color.Warn(annotation_name),
                                color.Warn(annotated_predicate),
                            ),
                            rule_text,
                        )
    ...
    def ExtractAnnotations(cls, rules, restrict_to=None, flag_values=None):
        ...
        for rule in rules:
            ...
            if rule_predicate in cls.ANNOTATING_PREDICATES:
                ...
                class Thrower(object):
                    def __contains__(self, key):
                        ...
                        raise rule_translate.RuleCompileException(
                            ...
                            "variable %s." % (color.Warn(key)),
                            rule_text,
                        )

                flag_values = flag_values or Thrower()
                ql = expr_translate.QL(
                    Thrower(), ThrowException, ThrowException, flag_values
                )
                ...
                if (
                    "0" in field_values
                    and isinstance(field_values["0"], dict)
...
class LogicaProgram(object):
    ...
    def __init__(self, rules, table_aliases=None, user_flags=None):
        ...
        library_rules = parse.ParseFile(
            dialects.Get(self.annotations.Engine()).LibraryProgram()
        )["rule"]
    ...
    def ExtractDollarParamsFromString(cls, s):
        ...
        return set(
            p
            for p in params  # Built-in, we shouldn't override them.
    ...
    def PredicateSql(self, name, allocator=None, external_vocabulary=None):
        ...
        if len(rules) == 1:
            ...
            rules_sql = [
                "\n".join("  " + split_value for split_value in r.split("\n"))
                for r in rules_sql
            ]
    ...
    def FunctionSql(self, name, allocator=None, internal_mode=False):
        ...
        s = rule_translate.ExtractRuleStructure(
            rule, external_vocabulary=None, names_allocator=allocator
        )

        udf_variables = [
            v if isinstance(v, str) else "col%d" % v
            for v in s.select
            if v != "logica_value"
        ]
        ...
        if internal_mode:
            return (
                "%s(%s)" % (name, ", ".join("{%s}" % v for v in udf_variables)),
                sql,
            )
    ...
    def FormattedPredicateSql(self, name, allocator=None):
        ...
        formatted_sql = (
            self.execution.flags_comment + defines_and_exports + FormatSql(sql)
        )
        if True:
            self.execution.preamble = self.UseFlagsAsParameters(self.execution.preamble)
        ...
        while sql != prev_sql:
            ...
            if num_subs > 100:
                raise rule_translate.RuleCompileException(
                    ...
                    "Flags:\n"
                    + "\n".join(
                        "--{0}={1}".format(*i) for i in self.flag_values.items()
                    ),
                )
            # Do the substitution!
    ...
    def RunInjections(self, s, allocator):
        ...
        while True:
            ...
            for table_name_rsql, table_predicate_rsql in s.tables.items():
                ...
                if (
                    len(rules) == 1
                    and ("distinct_denoted" not in rules[0])
                    and self.annotations.OkInjection(table_predicate_rsql)
                ):
                    [r] = rules
                    rs = rule_translate.ExtractRuleStructure(r, allocator, None)
                    ...
                    for (table_name, table_var), clause_var in s.vars_map.items():
                        if table_name != table_name_rsql:
                            ...
                            if table_var not in rs.select:
                                if "*" in rs.select:
                                    subscript = {
                                        "literal": {"the_symbol": {"symbol": table_var}}
                                    }
                                    s.vars_unification.append(
                                        {
                                            "left": {
                                                "variable": {"var_name": clause_var}
                                            },
                                            "right": {
                                                "subscript": {
                                                    "subscript": subscript,
                                    ...
                                    raise rule_translate.RuleCompileException(
                                        color.Format(
                                            ...
                                            dict(
                                                table_predicate_rsql=table_predicate_rsql,
                                                table_var=table_var,
    ...
    def SingleRuleSql(
        self, rule, allocator=None, external_vocabulary=None, is_combine=False
    ):
        ...
        s = rule_translate.ExtractRuleStructure(r, allocator, external_vocabulary)
...
class SubqueryTranslator(object):
    ...
    def TranslateTableAttachedToFile(self, table, ground, external_vocabulary):
        ...
        self.execution.dependency_edges.append(
            (table, self.execution.workflow_predicates_stack[-1])
        )
        ...
        if table in self.program.defined_predicates:
            ...
            export_statement = (
                ...
                + "CREATE TABLE {name} AS {dependency_sql}".format(
                    name=ground.table_name, dependency_sql=FormatSql(dependency_sql)
                )
            )
    ...
    def TranslateWithedTable(self, table):
        ...
        if table not in self.execution.table_to_defined_table_map:
            ...
            if (
                table
                not in self.execution.with_compilation_done_for_parent[parent_table]
            ):
                # Swap these lines to compile a recursive with.
                _ = self.program.PredicateSql(table, self.allocator)
                self.execution.with_compilation_done_for_parent[parent_table].add(table)
    ...
    def TranslateTable(self, table, external_vocabulary):
        ...
        if ground:
            return self.TranslateTableAttachedToFile(table, ground, external_vocabulary)
        if table in self.program.defined_predicates:
    ...
    def TranslateRule(self, rule, external_vocabulary, is_combine=False):
        return self.program.SingleRuleSql(
            rule, self.allocator, external_vocabulary, is_combine=is_combine
        )
