...
if "." not in __package__:
    from compiler.dialect_libraries import (
        bq_library,
        presto_library,
        psql_library,
        sqlite_library,
...
class SqLiteDialect(Dialect):
    ...
    def BuiltInFunctions(self):
        return {
            ...
            "Range": (
                "(select json_group_array(n) from (with recursive t as"
    ...
    def DecorateCombineRule(self, rule, var):
        ...
        rule["head"]["record"]["field_value"][0]["value"]["aggregation"]["expression"][
            "call"
                ...
                "call": {
                    ...
                    "record": {
                        "field_value": [
                            {
                                "field": 0,
                                "value": rule["head"]["record"]["field_value"][0][
                                    "value"
                                ]["aggregation"]["expression"]["call"]["record"][
                                    "field_value"
                                ][
                                    0
                                ][
...
class Trino(Dialect):
    ...
    def BuiltInFunctions(self):
        return {
            ...
            "ArrayConcat": "{0} || {1}",
            'Count': 'APPROX_DISTINCT(%s)'  # added this one | Robert
        }
