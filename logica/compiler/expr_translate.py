...
class QL(object):
    ...
    def __init__(
        ...
        self.built_in_infix_operators = copy.deepcopy(self.BUILT_IN_INFIX_OPERATORS)
    ...
    def CleanOperatorsAndFunctions(self):
        ...
        for d in [self.built_in_infix_operators, self.built_in_functions]:
    ...
    def InstallBulkFunctionsOfStandardSQL(cls):
        ...
        for row in reader:
            ...
            cls.BULK_FUNCTIONS[function_name] = "%s(%s)" % (row["sql_function"], "%s")
            cls.BULK_FUNCTIONS_ARITY_RANGE[function_name] = (
                int(row["min_args"]),
                float("inf")
                if row["has_repeated_args"] == "1"
                else int(row["max_args"]),
            )
    ...
    def ListLiteralInternals(self, literal):
        return ", ".join([self.ConvertToSql(e) for e in literal["element"]])
    ...
    def ConvertRecord(self, args):
        ...
        for f_v in args["field_value"]:
            assert "expression" in f_v["value"], "Bad record: %s" % args
    ...
    def Record(self, record):
        ...
        if self.dialect.Name() == "SqLite":
            arguments_str = ", ".join(
                "'%s', %s"
                % (f_v["field"], self.ConvertToSql(f_v["value"]["expression"]))
                for f_v in record["field_value"]
            )
            return "JSON_OBJECT(%s)" % arguments_str
        arguments_str = ", ".join(
            "%s AS %s" % (self.ConvertToSql(f_v["value"]["expression"]), f_v["field"])
            for f_v in record["field_value"]
        )
    ...
    def GenericSqlExpression(self, record):
        ...
        if (
            "literal" not in record["field_value"][0]["value"]["expression"]
            or "the_string"
            not in record["field_value"][0]["value"]["expression"]["literal"]
        ):
            raise self.exception_maker(
                "SqlExpr must have first argument be string, got: %s" % top_record[0]
            )

        template = record["field_value"][0]["value"]["expression"]["literal"][
            "the_string"
        ]["the_string"]
    ...
    def ConvertAnalyticListArgument(self, expression):
        if "literal" not in expression or "the_list" not in expression["literal"]:
            raise self.exception_maker(
    ...
    def ConvertAnalytic(self, call):
        ...
        if len(call["record"]["field_value"]) != 3 + is_window:
            raise self.exception_maker(
                "Function %s must have %d arguments."
                % (call["predicate_name"], 3 + is_window)
            )
    ...
    def SubIfStruct(self, implication, subscript):
        ...
        all_records = all(
            ("record" in if_then["consequence"]) for if_then in implication["if_then"]
        )
    ...
    def ConvertToSql(self, expression):
        ...
        if "call" in expression:
            ...
            if call["predicate_name"] == "Cast":
                if (
                    len(arguments) != 2
                    or "literal"
                    not in call["record"]["field_value"][1]["value"]["expression"]
                    or "the_string"
                    not in call["record"]["field_value"][1]["value"]["expression"][
                        "literal"
                    ]
                ...
                cast_to = call["record"]["field_value"][1]["value"]["expression"][
                    "literal"
            ...
            if call["predicate_name"] == "FlagValue":
                if (
                    len(arguments) != 1
                    or "literal"
                    not in call["record"]["field_value"][0]["value"]["expression"]
                    or "the_string"
                    not in call["record"]["field_value"][0]["value"]["expression"][
                        "literal"
                    ]
                    ...
                    if not arity_range[0] <= len(arguments) <= arity_range[1]:
                        raise self.exception_maker(
                            color.Format(
                                ...
                                dict(ydg_f=ydg_f, a=arity_range, b=len(arguments)),
                            )
                        )
        ...
        if "subscript" in expression:
            ...
            if "implication" in sub["record"]:
                simplified_sub = self.SubIfStruct(
                    sub["record"]["implication"], subscript
                )
                if simplified_sub:
        ...
        assert (
            False
        ), "Logica bug: expression %s failed to compile for unknown reason." % str(
            expression
        )
