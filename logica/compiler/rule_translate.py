...
def ReplaceVariable(old_var, new_expr, s):
    """Replacing a variable in expression s."""
    ...
    for k in member_index:
        if (
            isinstance(s[k], dict)
            and "variable" in s[k]
            and s[k]["variable"]["var_name"] == old_var
        ):
            s[k] = new_expr
    if isinstance(s, dict):
        for k in s:
...
class NamesAllocator(object):
    ...
    def FunctionExists(self, function_name):
        return (
            function_name in expr_translate.QL.BasisFunctions()
            or function_name in self.custom_udfs
        )
...
class ExceptExpression(object):
    ...
    def Build(cls, table_name, except_fields):
        return "(SELECT AS STRUCT %s.* EXCEPT (%s))" % (
            table_name,
            ",".join(except_fields),
        )
...
class RuleStructure(object):
    ...
    def __init__(
        self, names_allocator=None, external_vocabulary=None, custom_udfs=None
    ):
    ...
    def OwnVarsVocabulary(self):
        ...
        return {
            k: TableAndFieldToSql(v[0], LogicaFieldToSqlField(v[1]))
    ...
    def SortUnnestings(self):
        ...
        unnesting_of = {u[0]["variable"]["var_name"]: u for u in self.unnestings}
        unnesting_variables = set(unnesting_of)
        depends_on = {
            u[0]["variable"]["var_name"]: set(
                AllMentionedVariables(u[1], dive_in_combines=True)
            )
            & unnesting_variables
                ...
                for k, r in [["left", "right"], ["right", "left"]]:
                    ...
                    if (
                        ...
                        and (
                            ...
                            or not str(u[k]["variable"]["var_name"]).startswith("x_")
                        )
                    ):
                        u_left = u[k]["variable"]["var_name"]
                        u_right = u[r]
                        if "variable" in u_right:
                            log_data = self.synonym_log.get(
                                u_right["variable"]["var_name"], []
                            )
                            log_data.append(u_left)
                            log_data.extend(self.synonym_log.get(u_left, []))
                            self.synonym_log[u_right["variable"]["var_name"]] = log_data
                ...
                if assert_full_ellimination:
                    if True:
                        if variables:
                            ...
                            # the user.
                            violators = {
                                v.split(" # disambiguated")[0] for v in violators
                            }
                            raise RuleCompileException(
                                color.Format(
                    ...
                    # the user.
                    unassigned_variables = {
                        v.split(" # disambiguated")[0] for v in unassigned_variables
                    }
    ...
    def AsSql(self, subquery_encoder=None, flag_values=None):
        ...
        for k, v in self.select.items():
            if k == "*":
                ...
                fields.append(
                    "%s AS %s" % (ql.ConvertToSql(v), LogicaFieldToSqlField(k))
                )
        r += ",\n".join("  " + f for f in fields)
        if self.tables or self.unnestings or self.constraints or self.distinct_denoted:
            r += "\nFROM\n"
            tables = []
            ...
            # Indent the from_str.
            from_str = "\n".join(
                "  " + split_value for split_value in from_str.split("\n")
            )
            r += from_str
            if self.constraints:
                r += "\nWHERE\n"
            ...
            if self.distinct_vars:
                ...
                if subquery_encoder.execution.dialect.GroupBySpecBy() == "name":
                    ...
                    r += ", ".join(
                        str(selected_fields.index(v) + 1) for v in ordered_distinct_vars
                    )
...
def ExtractPredicateStructure(c, s):
    ...
    for field_value in c["record"]["field_value"]:
        ...
        s.vars_unification.append(
            {"left": {"variable": {"var_name": var_name}}, "right": expr}
        )
...
def ExtractConjunctiveStructure(conjuncts, s):
    ...
    for c in conjuncts:
        if "predicate" in c:
            ...
            if (
                "variable" in c["unification"]["right_hand_side"]
                or "variable" in c["unification"]["left_hand_side"]
            ):
                s.vars_unification.append(
                    {
...
def HasCombine(r):
    ...
    if isinstance(r, dict):
        ...
        assert False, "HasCombine should be called on list or dict. Got: %s" % str(r)
    ...
    def Replace(tree, outer_variables):
        ...
        for v in introduced_variables:
            ...
            new_name = "%s # disambiguated with %s" % (
                v,
                names_allocator.AllocateVar("combine_dis"),
            )
            ...
            s.vars_unification.append(
                {
                    ...
                    "right": {
                        "variable": {
                            "var_name": names_allocator.AllocateVar(
