...
class Functors(object):
    ...
    def BuildDirectArgsOf(self):
        ...
        for functor, rules in self.rules_of.items():
            ...
            for rule in rules:
                if "body" in rule:
                    args |= self.BuildDirectArgsOfWalk(
                        rule["body"], ExtractPredicateName
                    )
                args |= self.BuildDirectArgsOfWalk(
                    rule["head"]["record"], ExtractPredicateName
                )
    ...
    def AllRulesOf(self, functor):
        ...
        for f in self.args_of[functor]:
            if f == functor:
                raise FunctorError(
                    "Failed to eliminate recursion of %s." % functor, functor
                )
    ...
    def MakeAll(self, predicate_to_instruction):
        ...
        while needs_building:
            ...
            for (new_predicate, instruction) in sorted(predicate_to_instruction):
                name, applicant, args_map = self.ParseMakeInstruction(
                    new_predicate, instruction
                )
                if (
                    new_predicate not in needs_building
                    or applicant in needs_building
                    or (set(args_map.values()) & needs_building)
                ):
                    continue
            ...
            if needs_building and not something_built:
                raise FunctorError("Could not resolve Make order.", str(needs_building))
        if False:
    ...
    def CollectAnnotations(self, predicates):
        ...
        for annotation, rules in self.rules_of.items():
            if annotation not in ["@Limit", "@OrderBy", "@Ground", "@NoInject"]:
                continue
            for rule in rules:
                if (
    ...
    def CallKey(self, functor, args_map):
        ...
        relevant_args = {k: v for k, v in args_map.items() if k in self.ArgsOf(functor)}
    ...
    def CallFunctor(self, name, applicant, args_map):
        ...
        rules = [
            ...
            for r in rules
            if (
                (args & self.args_of[r["head"]["predicate_name"]])
                or r["head"]["predicate_name"] == applicant
            )
        ]
        if not rules:
            raise FunctorError(
                "Rules for %s when making %s are not found" % (applicant, name), name
            )
