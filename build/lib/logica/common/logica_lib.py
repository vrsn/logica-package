...
if "." not in __package__:
    from compiler import functors, rule_translate, universe
...
def ParseOrExit(filename, import_root=None):
    ...
    try:
        parsed_rules = parse.ParseFile(program_text, import_root=import_root)["rule"]
...
def RunQuery(sql, settings=None, output_format="pretty", engine="bigquery"):
    """Run a SQL query on BigQuery."""
    settings = settings or {}
    if engine == "bigquery":
        p = subprocess.Popen(
            ["bq", "query", "--use_legacy_sql=false", "--format=%s" % output_format],
        ...
        p = subprocess.Popen(
            ["psql", "--quiet"], stdin=subprocess.PIPE, stdout=subprocess.PIPE
        )
    elif engine == "trino":
        catalog = settings.get("catalog", "memory")
        server = settings.get("server", "http://localhost:8080")
        p = subprocess.Popen(
            ["trino", "--catalog=%s" % catalog, "--server=%s" % server]
...
def RunPredicate(
    filename, predicate, output_format="pretty", user_flags=None, import_root=None
):
    """Run a predicate on BigQuery."""
    p = GetProgramOrExit(filename, user_flags=user_flags, import_root=import_root)
    sql = p.FormattedPredicateSql(predicate)
    engine = p.annotations.Engine()
    if (
        "@Engine" in p.annotations.annotations
        and engine in p.annotations.annotations["@Engine"]
    ):
        settings = p.annotations.annotations["@Engine"][engine]
    else:
        settings = {}
    return RunQuery(sql, settings, output_format, engine=engine)
